...
async function checkPort(port, host) {
  try {
    if (!_tcp) _tcp = (await import("tcp-port-used")).default;
    return await _tcp.check(port, host);
  } catch (_) {
...
export async function getSafeAddress() {
    ...
    let port = 8002
    // check initially
    let inUse = await checkPort(port, "localhost")
    // if in use, iterate and try again
    while (inUse) {
        port += 1
        inUse = await checkPort(port, "localhost")
    }
