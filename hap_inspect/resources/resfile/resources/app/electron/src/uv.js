...
import { isHarmonyOS, findNativePython, getPythonVersion, getPythonStrategy } from "./harmony.js";

// Lazy-load heavy deps (may not be available on HarmonyOS)
let unzip = null;
let untar = null;
async function loadUnzip() {
  if (!unzip) unzip = (await import("extract-zip")).default;
  return unzip;
}
async function loadTar() {
  if (!untar) untar = (await import("tar")).extract;
  return untar;
}
            ...
            await fetch(
                ...
                async blob => {
                    ...
                    if (path.extname(zipfile) === ".zip") {
                        // extract zip file...
                        const unzipFn = await loadUnzip();
                        await unzipFn(zipfile, {
                            dir: this.folder
                        })
                    }
                    if (path.extname(zipfile) === ".gz") {
                        // extract tar.gz file...
                        const untarFn = await loadTar();
                        untarFn({
                            file: zipfile,
                            cwd: this.folder,
                            strip: 1,
