...
import { e as electron, s as snapshot, r as resolve, I as Icon, a as asset, H as HasParams, p as python$1, l as status, k as html, n as devices, d as parsePath, w as writeFile, o as readFile, c as prefs, f as browseFileOpen, P as Param, j as profiles, i as pending, g as browseFileSave } from "./Theme.js";
import { g as goto } from "./client.js";
import { k as ssr_context, i as attr_style, c as bind_props, b as await_block, a as attr, d as attr_class, e as escape_html, g as getContext, j as setContext, f as ensure_array_like, h as derived, A as ATTACHMENT_KEY, l as run, m as hasContext, p as props_id, n as attributes, o as spread_props, s as stringify } from "./root.js";
import "./exports.js";
...
function exportExperimentToJS(experiment, options) {
  ...
  routines.forEach(function(r) {
    ...
    (r.components || []).forEach(function(c) {
      ...
      comps.push({ name: cn, tag, params: p });
    });
...
class Experiment {
  ...
  async runJS(compile = true) {
    ...
    if (!python$1 || !python$1.psychojs || typeof python$1.psychojs.browserRun !== "function") {
    ...
    try {
      ...
      if (officialJSPath) {
        ...
        for (const flowItem of this.flow?.flat || []) {
          if (typeof flowItem.addTerminator === "function") {
            ...
            try {
              if (python$1.psychojs.readConditions) {
                conditionsJSON = await python$1.psychojs.readConditions(condPath);
      ...
      const result = await python$1.psychojs.browserRun(finalJSCode, expName, conditionsJSON, officialJSPath ? JSON.stringify(resourceFiles) : "", officialJSPath ? expDir : "");
...
function Frame($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!--]--> <nav id="view-nav" class="svelte-158299m"><!--[-->`);
    const each_array = ensure_array_like(views);
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let view = each_array[$$index];
      $$renderer2.push(`<button${attr_class("nav-btn svelte-158299m", void 0, { "active": currentView === view })}>${escape_html(view.charAt(0).toUpperCase() + view.slice(1))}</button>`);
    }
    $$renderer2.push(`<!--]--></nav> `);
    if (ribbon) {
      $$renderer2.push("<!--[0-->");
...
function Dialog_1$2($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let valid = derived(() => Object.values(devices).every((element2) => Object.values(element2.params).every((param) => param.valid.value)));
    let restore = {
...
function DklColor($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { value = void 0, target = "param" } = $$props;
    let dkl = [90, 0, 1];
    let preview = derived(() => python$1.liaison.send(
      "app",
      {
        command: "init",
        args: [
...
function Notebook_1($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let horizontal = derived(() => element2.tag === "CodeComponent");
    let categOrder = {
...
function ParamsDialog($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let valid = derived(() => Object.values(element2.params).every((param) => param.valid.value));
    let btnsDisabled = derived(() => ({ OK: !valid(), APPLY: !valid() }));
    let $$settled = true;
    let $$inner_renderer;
    function $$render_inner($$renderer3) {
