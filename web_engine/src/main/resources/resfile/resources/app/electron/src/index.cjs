...
(async () => {
  ...
  // TODO: Python backend not yet adapted - stub handlers to skip download UI
  // Register IPC handlers so frontend can call them
  ipcMain.handle("python.uv.exists", () => true);
  ipcMain.handle("python.uv.folder", () => "/tmp/stub-uv");
  ipcMain.handle("python.uv.executable", () => "/tmp/stub-uv/uv");
  ipcMain.handle("python.uv.install", () => Promise.resolve(true));
  ipcMain.handle("python.uv.makeExecutable", () => Promise.resolve(true));
  ipcMain.handle("python.uv.findPython", () => Promise.resolve("/usr/bin/python3"));
  ipcMain.handle("python.uv.getEnvironments", () => Promise.resolve([]));
  ipcMain.handle("python.venv.setup", () => Promise.resolve(true));
  ipcMain.handle("python.venv.executable", () => "/tmp/stub-python");
  ipcMain.handle("python.venv.installPackage", () => Promise.resolve(true));
  ipcMain.handle("python.venv.uninstallPackage", () => Promise.resolve(true));
  ipcMain.handle("python.venv.getPackages", () => Promise.resolve([]));
  ...
  ipcMain.handle("python.shell.close", () => Promise.resolve(true));
