...
// ── liaison Python 后端日志全同步到左上角 terminal ───────────────
// 早注册也没事：panel 未创建时 appendTermOutput 静默返回，panel 创建后的事件正常显示
// payload 可能是 string（ liaison stdout）或 {error: "string"}（liaison stderr / RCP error 帧）
function _formatPayload(data) {
  if (data == null) return '';
  if (typeof data === 'string') return data;
  if (data instanceof Error) return data.message || String(data);
  if (data && typeof data === 'object') {
    // liaison stderr / RCP error 帧形如 {error: "..."} 或 {error: {message, traceback}}
    const e = data.error;
    if (typeof e === 'string') return e;
    if (e && typeof e === 'object') return (e.message || '') + (e.traceback ? '\n' + e.traceback : '');
    if (typeof data.message === 'string') return data.message;
    try { return JSON.stringify(data); } catch (_) { return String(data); }
  }
  return String(data);
}
ipcRenderer.on('stdout',        (evt, data) => appendTermOutput(_formatPayload(data), 'var(--text, #cdd6f4)'));
ipcRenderer.on('stderr',        (evt, data) => appendTermOutput(_formatPayload(data), 'var(--red, #f38ba8)'));
ipcRenderer.on('uv',            (evt, data) => appendTermOutput(_formatPayload(data), 'var(--subtext0, #a6adc8)'));
ipcRenderer.on('liaison:error', (evt, data) => appendTermOutput(_formatPayload(data), 'var(--red, #f38ba8)'));
